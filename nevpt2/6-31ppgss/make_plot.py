...
import sys
sys.path.append('../src/')
from plot import make_empty_elist, morse, find_minimum, fill_panel, append_energies_f, c_list, kcal
...
for R in R_list:
    E_anion   = append_energies_f('anion',R,E_anion)
    E_radical = append_energies_f('radical',R,E_radical)
...
fill_panel(ax1,'$R_{\mathrm{OH}} \, [\mathrm{\AA}]$',[0.8,1.2],[0.8,0.9,1.0,1.1,1.2],['0.8','0.9','1.0','1.1','1.2'],
               '$E_{\mathrm{anion}} \, [E_h]$',[-75.62,-75.34],[-75.62,-75.55,-75.48,-75.41,-75.34],
                                                  ['-75.62','-75.55','-75.48','-75.41','-75.34'])
fill_panel(ax2,'$R_{\mathrm{OH}} \, [\mathrm{\AA}]$',[0.8,1.2],[0.8,0.9,1.0,1.1,1.2],['0.8','0.9','1.0','1.1','1.2'],
               '$E_{\mathrm{radical}} \, [E_h]$',[-75.62,-75.34],[-75.62,-75.55,-75.48,-75.41,-75.34],
                                                  ['-75.62','-75.55','-75.48','-75.41','-75.34'])
fill_panel(ax3,'',[0,8],[0,1,2,3,4,5,6,7,8],['SCF','$R_y$/IAO','q-UCCSD/IAO','FCI/IAO','NEVPT2($R_y$,QSE)','NEVPT2(q-UCCSD,QSE)','NEVPT2(FCI,QSE)','NEVPT2(FCI,FCI)','FCI'],
                  '$E_{\mathrm{radical}}-E_{\mathrm{anion}}$ [kcal/mol]',[-10,40],[-10,0,10,20,30,40][::-1],[-10,0,10,20,30,40][::-1])
