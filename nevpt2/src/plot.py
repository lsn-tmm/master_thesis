...
def append_energies_f(species,R,E_list, path='./'):
    f = open('%s/%s/R_%s/results.txt'%(path,species,R),'r')
    f = f.readlines()
    f = [x.split() for x in f]
    E_list['full']['scf'].append(float(f[0][5]))
    E_list['iao']['fci'].append(float(f[1][5]))
    E_list['full']['fci'].append(float(f[3][5]))
    E_list['iao']['Ry'].append(float(f[21][3]))
    E_list['iao']['qUCCSD'].append(float(f[19][2]))
    E_list['iao']['nevpt2(fci,fci)'].append(float(f[4][5]))
    E_list['iao']['nevpt2(fci,qse)'].append(float(f[5][5]))
    E_list['iao']['nevpt2(qUCCSD,qse)'].append(float(f[18][5]))
    E_list['iao']['nevpt2(Ry,qse)'].append(float(f[20][5]))
    return E_list
...
def append_energies_cbs_f(species,basis,R,E_list):
    f = open('../%s/%s/R_%s/results.txt'%(basis,species,R),'r')
    f = f.readlines()
    f = [x.split() for x in f]
    E_list['full']['scf'].append(float(f[0][5]))
