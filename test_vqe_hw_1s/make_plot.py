...
for lin in lindep:
    for hw in hardware:
        for s in ['anion','radical']:
            for p in ['em_re']:
                data['%s/%s/%s_%s' % (lin,s,hw,p)] = []
                err['%s/%s/%s_%s' % (lin,s,hw,p)] = []
    ...
    for hw in hardware:
        for s in ['anion','radical']:
            for R in ['0.80','0.90','1.00','1.10','1.20']:
                for p in ['em_re']:
                    f = open('%s/%s/%s/R_%s_%s_%s/results_vqe.txt' % (lin,s,R,R,hw,p),'r')
                    f = [x.split() for x in f.readlines()]
                    f = f[len(f)-1]
                    ave = float(f[8])
                    std = float(f[10])
                    data['%s/%s/%s_%s' % (lin,s,hw,p)].append(ave) 
                    err['%s/%s/%s_%s' % (lin,s,hw,p)].append(std)
                
                         
for s in ['anion','radical']:
    for R in ['0.80','0.90','1.00','1.10','1.20']:
        f = open('0.5/%s/%s/R_%s/results.txt' % (s,R,R),'r')
        f = [x.split() for x in f.readlines()]
        f = f[len(f)-2]
        ave = float(f[3])
        data['%s' % (s)].append(ave)
...
colors = [ [c_list['red'],c_list['orange']],[c_list['dark_blue'],c_list['cerulean']],[c_list['brown'],c_list['earwax']],[c_list['dark_green'],c_list['mid_green']]]
markers = [ ['o','+'],['o','+'],['o','+'],['o','+']]
...
for hw,col,mrk in zip(hardware,colors,markers):
    #x+=8
    for p,lin,c,m in zip(['em_re','em_re'],lindep,col,mrk):
        radical = np.array(data['%s/radical/%s_%s' % (lin,hw,p)])
        anion   = np.array(data['%s/anion/%s_%s' % (lin,hw,p)])
        dr      = np.array(err['%s/radical/%s_%s' % (lin,hw,p)])
        da      = np.array(err['%s/anion/%s_%s' % (lin,hw,p)])
        E       = radical - anion
        dE      = np.sqrt(dr**2+da**2)
        l = ''
        for word in hw.split('_'):
            l = l + word + ' '
        if   (p == 'em_re') : l = l +'EM+RE ' + lin
        elif (p == 'em') : l = l +'EM ' + lin
        elif (p == 'raw') : l = l +'RAW ' + lin
...
fill_panel(ax3,'',[0,9],[1,2,3,4,5,6,7,8],['kolkata 0.5','kolkata 0.1','auckland 0.5','auckland 0.1','qasm kolkata 0.5','qasm kolkata 0.1','qasm auckland 0.5','qasm auckland 0.1'],
                  '$E_{\mathrm{radical}}-E_{\mathrm{anion}}$ [kcal/mol]',[-8,8],[-8,-5,-1,2,5,8],[-8,-5,-1,2,5,8])

fill_panel(ax6,'',[0,9],[1,2,3,4,5,6,7,8],['kolkata 0.5','kolkata 0.1','auckland 0.5','auckland 0.1','qasm kolkata 0.5','qasm kolkata 0.1','qasm auckland 0.5','qasm auckland 0.1'],
