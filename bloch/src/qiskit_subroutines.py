...
def prepare_operators(SOLVER,quantum_variables,read_from_file=False,write_to_file=False,filename=None):
    if(read_from_file): # if the preparation has been carried out and the results are stored in a file, read that file
       x = np.load(filename,allow_pickle=True).item()
       oper,quantum_variables = x['oper'],x['quantum_variables']
    else:               # otherwise, compute everything
       ...
       for k in ['u','d','uu','dd','ud']:
           ...
           print("qse operators prepared for ",k)
       # -----
       H_op,z2syms,sqlist,target_sector = taper_principal_operator(H_op,target_sector=quantum_variables.target_sector)
       print("after tapering qubits(H) = ",H_op.num_qubits)
       oper['hamiltonian'] = H_op
       for k in oper.keys():
           if(k!='hamiltonian'):
              oper[k] = taper_auxiliary_operators(oper[k],z2syms,target_sector)
           print("qse operators tapered for ",k)
       quantum_variables.target_sector = target_sector
       quantum_variables.tapering_info = [z2syms,sqlist]
    if(write_to_file): # if the results need to be stored in a file, write them on such a file
       np.save(filename,{'oper':oper,'quantum_variables':quantum_variables},allow_pickle=True)
...
def measure_operators_from_bloch_vector(operators,wfn_circuit,bloch):
    import sys
    #sys.path.append('../../../../tomography/')
    sys.path.append('../../../../../tomography/') # only nevpt2_hw
    from utilities import pauli_label_to_index
    ...
    for idx,oper in enumerate(operators):
        if(not oper.is_empty()):
           ...
           for c,P in oper._paulis:
               jP = pauli_label_to_index(P.to_label())
               ave +=  c.real*bloch[jP,0]
               std += (c.real*bloch[jP,1])**2
           std = np.sqrt(np.abs(std))
           results_list.append([ave,std])
        else:
           results_list.append([0,0])
    return results_list
