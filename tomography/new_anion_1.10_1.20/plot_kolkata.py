...
kolkata_em    = np.load('kolkata_em_bloch.npy',allow_pickle=True).item()
kolkata_em_re = np.load('kolkata_em_re_extrapolated_bloch.npy',allow_pickle=True).item()
kolkata_raw   = np.load('kolkata_raw_bloch.npy',allow_pickle=True).item()
...
for (x,y),k in zip([(0,0),(1,0),(2,0),(3,0),(4,0),(0,1),(1,1),(2,1),(3,1),(4,1)],A.keys()):
    ...
    a3 = filter(kolkata_raw[k],4)
    a4 = filter(kolkata_em[k],4)
    a5 = filter(kolkata_em_re[k],4)
    ...
    axs[x,y].errorbar(pa,a3[:,0]-a1[:,0],yerr=a3[:,1],label='kolkata RAW',marker='x', ms=5, ls='none', c='#DC343B')
    axs[x,y].errorbar(pa,a4[:,0]-a1[:,0],yerr=a4[:,1],label='kolkata EM',marker='1', ms=5, ls='none', c='#FF8856')
    axs[x,y].errorbar(pa,a5[:,0]-a1[:,0],yerr=a5[:,1],label='kolkata EM+RE',marker='2',ms=6,ls='none', c='#228B22')
...
plt.savefig('bloch_kolkata.pdf',format='pdf')
