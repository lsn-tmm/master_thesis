...
if(task=='submit'):
   T.submit_probability_distribution_jobs(circuits_per_job=max_circuits_per_job,logfile_name='logfile_'+device+'_em_re.txt')
elif(task=='retrieve'):
   jobs = [f.split()[2] for f in open('logfile_'+device+'_em_re.txt','r').readlines()]
   T.compute_probability_distributions(jobs,max_circuits_per_job,mitigated_counts=True)
   T.compute_bloch_vector()
   T.save_calculation(device+'/tomo_HW_em_re')
