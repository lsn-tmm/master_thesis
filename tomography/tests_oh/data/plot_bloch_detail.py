...
bloch_ideal    = np.load('data/tomo_statevector_bloch.npy',allow_pickle=True).item()
bloch_hw       = np.load('data/tomo_HW_raw_bloch.npy',allow_pickle=True).item()
bloch_hw_em    = np.load('data/tomo_HW_em_bloch.npy',allow_pickle=True).item()
bloch_hw_em_na = np.load('data/tomo_HW_em_re_bloch.npy',allow_pickle=True).item()
bloch_hw_em_re = np.load('data/tomo_HW_em_re_extrapolated_bloch.npy',allow_pickle=True).item()
A = np.load('circuits.npy',allow_pickle=True).item()
...
for (x,y),k in zip([(0,0),(0,1),(1,0),(1,1)],A.keys()):
    ...
    pa = [x for x in range(a1.shape[0])]
    axs[x,y].plot(pa,a1[:,0]-a1[:,0],label='exact',marker='o',ls='none',ms=5)
    axs[x,y].errorbar(pa,a3[:,0]-a1[:,0],yerr=a3[:,1],label='hw, RAW',marker='x',ls='none')
    axs[x,y].errorbar(pa,a4[:,0]-a1[:,0],yerr=a4[:,1],label='hw, EM',marker='1',ls='none')
    axs[x,y].errorbar(pa,a5[:,0]-a1[:,0],yerr=a5[:,1],label='hw, EM+RE',marker='2',ms=3,ls='none')
    axs[x,y].text(128,0.15,k.replace('_',' '),ha='center')
    axs[x,y].set_xlabel(r'index $\mu$')
    axs[x,y].set_ylabel(r'$\mbox{Tr}[\rho P_\mu] - \mbox{Tr}[\rho_{exact} P_\mu]$')
...
lgd = axs[0,0].legend(handles,labels,ncol=5,fancybox=True,shadow=True,
                 bbox_to_anchor=(x0L,y0L,dxL,dyL),loc=3,mode='expand',
                 borderaxespad=0,handlelength=2)
plt.savefig('bloch_hw.pdf',format='pdf')
'''
