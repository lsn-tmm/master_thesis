...
T = Hardware_Tomography(circuits=circuits,shots=20000,optimization_level=0,
                        device_name='ibmq_manila',initial_layout=[1,2,3,4],provider=provider,
...
if(task=='submit'):
   T.submit_probability_distribution_jobs(circuits_per_job=max_circuits_per_job,logfile_name='logfile_manila_em_re.txt')
elif(task=='retrieve'):
   jobs = [f.split()[2] for f in open('logfile_manila_em_re.txt','r').readlines()]
