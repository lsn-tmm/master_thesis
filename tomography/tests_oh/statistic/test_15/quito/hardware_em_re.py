...
T = Hardware_Tomography(circuits=circuits,shots=20000,optimization_level=0,
                        device_name='ibmq_quito',initial_layout=[2,1,3,4],provider=provider,
...
if(task=='submit'):
   T.submit_probability_distribution_jobs(circuits_per_job=max_circuits_per_job,logfile_name='logfile_quito_em_re.txt')
elif(task=='retrieve'):
   jobs = [f.split()[2] for f in open('logfile_quito_em_re.txt','r').readlines()]
