...
class VQE_QSE_solver(NEVPT2_solver):
      ...
      def set_quantum_variables(self,quantum_variables,with_bloch_vector=False,bloch_vector=None):
      ...
      def compute_excited_states(self,ground_state):
          ...
          for k in ['u','d','uu','dd','ud']:
              ...
              for m,(a,b) in enumerate([(a,b) for a in range(n_op) for b in range(n_op) if a<=b]):
                  ...
                  H[b,a] = Hk[m][0]
              sgm,V = LA.eigh(S)
              print("overlap matrix condition number %.12f %.12f " % (min(sgm),max(sgm)))
              e,U = safe_eigh(H,S)[:2]
