class Born_Oppenheimer:
    ...
    def do_scf(self, rho_0=None):
        ...
        if(self._na==self._nb):
           ...
           mf._eri       = ao2mo.restore(4,self._h2,n)
           if(rho_0 is None):
              rho_0 = np.zeros((n,n))
    ...
    def dump_on_file(self,fname='dictionary_output'):
        ...
        h2 = ao2mo.restore(1,mf._eri,C.shape[1])
        h2 = np.einsum('prqs,pi->irqs',h2,C)
        h2 = np.einsum('irqs,rj->ijqs',h2,C)
        h2 = np.einsum('ijqs,qk->ijks',h2,C)
        h2 = np.einsum('ijks,sl->ijkl',h2,C)
